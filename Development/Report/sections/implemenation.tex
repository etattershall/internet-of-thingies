...
We implemented a test bed for future development of a security protocol. Our proof-of-concept used standard IoT components to produce a system that moved data over the different levels of our architecture, demonstrating common features of a larger IoT system.
...
\paragraph{}
Depending on the use case, different communication technologies may be more applicable. If Edge Devices are relatively fixed, our serial protocol worked over USB - where there was no delay to scanning but was it was not wireless. Some protocols that are wireless - such as Zigbee - can connect a larger number of devices but require bespoke hardware at both ends. Our technique of sending a JSON encoded packet could work on any text communication protocol - we implemented it over USB and Bluetooth serial.

\paragraph{}
Edge devices interacted directly with the sensor, sending JSON packets of tagged data for the Smart Agent to forward to the broker. They were programmed in C which was compiled and uploaded using the Arduino IDE.


\begin{figure}
    \centering
    \includegraphics[width=0.7\textwidth]{setup.jpg}
    \caption{A Raspberry PI (bottom) connected to an Arduino (top) relaying data passed to the MQTT broker in the Cloud from the Arduino after reading the attached sensors.}
    \label{fig:setup}
\end{figure}
